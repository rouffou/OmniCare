...
- `src/Modules/Patients/`, `src/Modules/Agenda/` et `src/Modules/Billing/` — modules Vertical Slice (`Domain/`, `Features/<UseCase>/`, `Infrastructure/Persistence/`), chacun avec son composition root (`PatientsModule.cs`, `AgendaModule.cs`, `BillingModule.cs` : `Add<Module>Module()` + `Map<Module>Module()`). Billing : commandes idempotentes (`IIdempotentRequest` + en-tête HTTP `Idempotency-Key`), client MyCareNet factice (`FakeMyCareNetService`) en attendant l'intégration réelle.
