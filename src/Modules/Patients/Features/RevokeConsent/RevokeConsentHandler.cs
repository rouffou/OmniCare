...
public class RevokeConsentHandler : ICommandHandler<RevokeConsentCommand, Result<Guid>>
{
    private readonly IPatientsDbContext _context;

    public RevokeConsentHandler(IPatientsDbContext context)
    {
        _context = context;
    }

    public async Task<Result<Guid>> Handle(RevokeConsentCommand request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            patient.RevokeConsent(consentType);
        }
        catch (DomainException ex)
        {
            // Fiche archivée, ou aucun consentement actif de ce type.
